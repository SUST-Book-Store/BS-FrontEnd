...
<template>
  <div>
      ...
      <el-form-item label="封面" >
                <el-input v-model="book.photo" v-if="false"></el-input>
                <el-upload
...
<!-- 显示多个图片并且编辑 -->
  ...
  export default {
   ...
   methods: {
    handleUploadSuccess(index, response, file) {
  ...
  handleRemoveImage(index) {
    ...
    this.book.detail[index].fileList = []; // 清空对应索引的文件列表
        ...
        saveBook()
        {
          ...
          request.post('http://127.0.0.1:3000/admin/books/save', this.book) .then(res => {
  ...
  created() {
    ...
    // 发送请求获取书籍信息
    request.post('http://127.0.0.1:3000/admin/getBooks?id='+bookId) .then(res => {
